...
export class HttpService {
  ...
  saveMetube(metubeUrl: string): Observable<SettingsModel> {
    return this._http
      .post<SettingsModel>('/api/metube', { metubeUrl })
      .pipe(tap((settings) => this._storage.settings.set({ ...settings, metubeUrl })));
  }
  ...
  getChannel(id: number): Observable<ChannelModel | null> {
    return this._http.get<ChannelModel>(`/api/channels/${id}`).pipe(catchError(async () => null));
  }
