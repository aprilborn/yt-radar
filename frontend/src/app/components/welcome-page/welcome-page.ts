...
import { delay, filter, finalize, switchMap, take, tap } from 'rxjs';
import { MetubeValidatorDirective } from '../../shared/directives';
import { clearTextEffect, textWriterEffect } from '../../shared/helpers';
import { HttpService, SnackbarService, SnackbarType } from '../../shared/services';
...
export class WelcomePage implements OnInit {
  ...
  ngOnInit() {
    this._httpService
      .getSettings()
      .pipe(tap((settings) => (settings?.metubeUrl?.length ? this._router.navigate(['/']) : null)))
      .subscribe();
  ...
  private _onSuccess() {
    return this.metubeControl.statusChanges.pipe(
      ...
      take(1),
      switchMap(() => this._httpService.saveMetube(this.metubeControl.value)),
    );
  }
}
